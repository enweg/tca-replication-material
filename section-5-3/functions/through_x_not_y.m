...
% variable but not another at horizons h.
%
% Inputs:
...
function transmission_effect=through_x_not_y(M_, B, Oomega, idx_x, idx_y, k, h)

  if length(h) > 1
    transmission_effect = through_x_not_y(M_, B, Oomega, idx_x, idx_y, k, h(1));
    for i=2:length(h)
      tmp = through_x_not_y(M_, B, Oomega, idx_x, idx_y, k, h(i));
      transmission_effect = cat(3, transmission_effect, tmp);
    end
    return
  end

  % First term: Q[and_{t=0}^h not y_t]
  B_tilde = B; 
  Oomega_tilde = Oomega;
  for t=0:h
    B_tilde(idx_y+(t*k), :) = 0; 
  ...
  % Second term: Q[and_{t=0}^h not x_t and not y_t]
  B_tilde = B; 
  Oomega_tilde = Oomega;
  for t=0:h
    B_tilde(idx_y+(t*k), :) = 0; 
