...
% 1. If the `horizon` was changed in file `run003_sw2007.m`, then adjust 
...
horizon = 20;  
df = readtable(sprintf("output/effects-horizons-%d-alternative.csv", horizon));
...
filename = sprintf("plots/smets-wouters-decomposition-horizon-%d-alternative.pdf", horizon);
saveas(fig, filename);
